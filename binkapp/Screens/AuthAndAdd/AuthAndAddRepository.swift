...
class AuthAndAddRepository: WalletServiceProtocol {
    ...
    private func mapCustomCardToCoreData(membershipCard: MembershipCardPostModel, completion: @escaping (CD_MembershipCard?) -> Void) {
        Current.database.performBackgroundTask { context in
            ...
            let cardModel = CardModel(apiId: nil, barcode: barcode, membershipId: barcode, colour: colorPallete.primary, secondaryColour: colorPallete.secondary, merchantName: merchantName)
