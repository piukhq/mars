...
class SettingsViewModelTests: XCTestCase {
    ...
    func test_rowsCount_returnsCorrectCountForSupportSection() {
        XCTAssertEqual(sut.rowsCount(forSectionAtIndex: 1 + appearanceSection), 3)
    }
    ...
    func test_rowsCount_returnsCorrectCountForLegalSection() {
        XCTAssertEqual(sut.rowsCount(forSectionAtIndex: 3 + appearanceSection), 2)
    }
    
    #if DEBUG
    func test_rowsCount_returnsCorrectCountForDebugSection() {
        XCTAssertEqual(sut.rowsCount(forSectionAtIndex: 4 + appearanceSection), 1)
    }
    #endif
    ...
    func test_titleForSection_returnsCorrectTitleForSupportSection() {
        XCTAssertEqual(sut.titleForSection(atIndex: 1 + appearanceSection), "settings_section_support_title".localized)
    }
    
    func test_titleForSection_returnsCorrectTitleForAboutSection() {
        XCTAssertEqual(sut.titleForSection(atIndex: 2 + appearanceSection), "settings_section_about_title".localized)
    }
    
    func test_titleForSection_returnsCorrectTitleForLegalSection() {
        XCTAssertEqual(sut.titleForSection(atIndex: 3 + appearanceSection), "settings_section_legal_title".localized)
    }
    
    #if DEBUG
    func test_titleForSection_returnsCorrectTitleForDebugSection() {
        XCTAssertEqual(sut.titleForSection(atIndex: 4 + appearanceSection), "settings_section_debug_title".localized)
    }
    #endif
